'''
In this version, negative tokens are under-represented. 
...
📊 Pattern Statistics:
   Rejection patterns: 81
   Pending patterns: 52
   Approval stems: 42
✅ Token verification complete:
   Approval tokens: 759
   Rejection tokens: 219
   Pending tokens: 881

Likely because:

Positive patterns are general, negative patterns are less so. 
Suggestion: expand negation prefixes and positive-stems to interact them with.
...
if 'approval_token_ids' not in locals() or approval_token_ids is None:
    ...
    def find_approval_rejection_tokens(tokenizer):
        ...
        rejection_base_patterns = [
            ...
            'reject', 'decline', 'deny', 'refuse', 'disapprov',
            ...
            # Risk-related rejections
            'risky', 'unsafe', 'problematic', 'concerning', 'warning',
            
            # Financial rejection terms
            'default', 'bankrupt', 'insolvent', 'delinquent', 'overdue', 'foreclos',
            
            # Explicit negatives
            'impossible', 'forbidden', 'prohibited', 'banned', 'blocked', 'terminated', 'cancelled'
        ]
        
        # Add negation variants for positive terms
        negation_prefixes = ['un', 'in', 'non', 'dis', 'im']
        positive_stems = ['qualified', 'suitable', 'acceptable', 'favorable', 'viable', 'approved', 'adequate', 'sufficient', 'satisfactory']
