...
def prepare_regression_controls(metadata):
    ...
    # 1. Credit score buckets (e.g., "600-649", "650-699", "700-749", etc.)
    credit_dummies = pd.get_dummies(metadata['credit_score'], prefix='credit', drop_first=True)
    dummies_list.append(credit_dummies)
    feature_names.extend(credit_dummies.columns.tolist())
    
    # 2. LTV ratio buckets (e.g., "0-20%", "20-40%", etc.)
    ltv_dummies = pd.get_dummies(metadata['ltv_ratio'], prefix='ltv', drop_first=True)
    ...
    # 4. Income buckets (e.g., "$60k-80k", "$80k-100k", etc.)
    ...
    loan_dummies = pd.get_dummies(metadata['loan_amount'], prefix='loan', drop_first=True)
