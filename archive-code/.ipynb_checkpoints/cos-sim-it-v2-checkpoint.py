...
OUTPUT_DIR  = Path("/workspace/outputs/steering_analysis/v2")
...
ALPHAS            = [0.0, 0.5, 1.0, 2.5, 5.0, 10.0, 20.0, 22.5, 25]
...
class BiasInterrogator:
    ...
    def __init__(self, model_id, label="model"):
        ...
        # Validate grammar processor works correctly
        self._validate_grammar_processor()
    ...
    def _validate_grammar_processor(self):
        """Sanity check: ensure grammar processor only outputs A or B"""
        test_prompt = "Choose A or B: "
        inputs = self.tokenizer(test_prompt, return_tensors="pt").to(DEVICE)
        grammar = self._create_grammar_processor()
        ...
        for trial in range(10):
            out = self.model.generate(
                **inputs, 
                ...
                do_sample=True,  # Test stochastic sampling too
                temperature=1.0,
            )
            token = self.tokenizer.decode(out[0][-1]).strip()
            assert token in ["A", "B"], f"Trial {trial}: Grammar failed, got '{token}'"
        
        print("✓ Grammar processor validated (10/10 trials passed)")
    ...
    def run_steering_sweep(self, test_df, race_vector, layers, alphas):
        """
        Sweep over (layer, alpha) and compute flip rates.

        Strategy: For each White-approved applicant, steer with the
        Black direction and measure approve→deny flips.

        Returns:
            heatmap: (n_layers, n_alphas) flip rate matrix
            details: list of dicts with per-example results
        """
        # First get baselines for White applicants
        ...
        # Filter to those originally approved
        approved_ids = [pid for pid, dec in baselines.items() if dec == "A"]
        approved_df = white_df[white_df["pair_id"].isin(approved_ids)]
        print(f"  {len(approved_df)} approved at baseline (testing flip rate on these)")

        if len(approved_df) == 0:
            print("WARNING: No approved baselines found. Using all White applicants.")
        ...
        for i, layer in enumerate(tqdm(layers, desc="Steering layers")):
            for j, alpha in enumerate(alphas):
                ...
                for _, row in approved_df.iterrows():
                    # Use race_vector[layer+1] because index 0 = embedding layer
...
if __name__ == "__main__":
    ...
    # Plots
    plot_steering_heatmap(heatmap, LAYERS_TO_STEER, ALPHAS, save_path=OUTPUT_DIR / "steering_heatmap.png")
