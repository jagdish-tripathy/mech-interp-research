...
OUTPUT_DIR  = Path("/workspace/outputs/steering_analysis/v1")
...
class BiasInterrogator:
    ...
    def steer_and_decide(self, row, race_vector, layer_idx, alpha):
        ...
        the given layer and return the model's A/B decision.
        """
        prompt = build_prompt(row)
        inputs = self.tokenizer(prompt, return_tensors="pt").to(DEVICE)
...
if __name__ == "__main__":
    ...
    print(f"Array lengths: info_mag={len(info_mag)}, mean_cos={len(mean_cos)}")
    print(f"Last 5 magnitude values: {info_mag[-5:]}")
    print(f"Last 5 cosine values: {mean_cos[-5:]}")
    print(f"Minimum cosine value: {mean_cos.min()} at index {mean_cos.argmin()}")
    print("Done.")